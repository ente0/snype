...
| `-t`, `--term-mode` | `auto\|xterm\|tmux\|pty` | Store the terminal-backend preference used by the legacy launcher components. The current Monitor view is inline. |
| `--duration` | `SECONDS` | Accepted for CLI compatibility; the current Deauth view starts at 10 seconds and is adjusted with `+` / `-`. |
| `--dry-run` | | Avoid command execution in Scan and standalone Deauth. Monitor and Crack do not currently implement dry-run mode. |
...
5. **Crack** (`c`) — choose a capture with `p`, choose a wordlist with `w`, select the cracking tool with `t`, then press `Enter`.
